...
string_array_t* str_array_new() {
    string_array_t* array = (string_array_t*) malloc( sizeof( string_array_t ) );
...
void str_array_push(string_array_t* array, const char* str) {
    if( array->buffer == 0 )
    {
        array->buffer = (char**) malloc( sizeof(char*) );
    } else {
        array->buffer = (char**) realloc(array->buffer, sizeof(char*) * (array->size + 1) );
    }

    array->buffer[array->size] = (char*) malloc( strlen( str ) + 1);
...
void str_push(string_t* str, const char* buffer) {
    ...
    if( str->buffer == 0 )
    {
        str->buffer = (char*) malloc( length + 1 );
